...
def main(args):
    ...
    train_set, val_set, train_loader, val_loader = get_dataloader(
        str(DatasetInfo.data_dir), Processing(train_compose), dataset_rate=0.8, batch_size=30, num_workers=24
    )

    test_set, loader = get_test_dataloader(str(DatasetInfo.data_dir), Processing(test_compose), batch_size=30, num_workers=24)
    ...
    for epoch in range(total_epoch):  # loop over the dataset multiple times
        ...
        with torch.no_grad():
            for inputs,labels in val_loader:
                ...
                if (i + 1) % print_per_iteration == 0:
                    print('Validation Loss', sum_loss)
